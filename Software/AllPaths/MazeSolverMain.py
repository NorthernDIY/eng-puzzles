# -*- coding: utf-8 -*-
"""
Created on Fri Feb  4 22:44:47 2022
...
if use_default_image:
    opencv_image = cv2.imread('maze.png')
...
if marked_image is not None:
    if st.button('Solve Maze'):
        with st.spinner('Solving your maze'):
            path = MazeSolver.find_longest_path(opencv_image,(start_x, start_y),(finish_x, finish_y))
        pathed_image = opencv_image.copy()
        path_thickness = (pathed_image.shape[0]+pathed_image.shape[0])//2//100
        MazeSolver.drawPath(pathed_image, path, path_thickness)
